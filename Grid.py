...
class Grid:
    ...
    def utility(self) -> int:
        ...
        # 5 all huerisitcs with weight

        # count = 0
        # sum = 0
        # for i in range(4):
        #     for j in range(4):
        #         sum += self.matrix[i][j] * (1.1) # 1 sum of tiles / num of tiles
        #         if (i==0 or i==3 or j==0 or j==3):
        #             sum += self.matrix[i][j] * (1.3) # 2 big tiles in border
        #         if (i==0 and j==0) or (i==3 and j==0) or (i==0 and j==3) or (i==3 and j==3):
        #         # 4 sum of corners
        #             sum += self.matrix[i][j] * (1.4)
        #         if self.matrix[i][j] != 0:
        #             count += 1
